<template>
  <div class="relative py-8">
    <template v-for="item in testValue" :key="item">
      <input v-model="testResult" type="checkbox" :value="item" @change="test(item)" />
      <span>{{ item }}</span>
    </template>
    <div>{{ testResult }}</div>
    <!-- <div
    ...
    <div class="container relative max-w-2xl mx-auto bg-white shadow-xl shadow-slate-700/10 ring-1 ring-gray-900/5">
      ...
      <main>
        <div class="p-4 mx-auto prose md:px-6 prose-indigo sm:rounded-md">
          ...
          <p> The idea of this template is to bundle as many often used and useful features pre-configured as possible. If you don't need something, just remove it! </p>
          ...
          <p>
            ...
            is recommended to take full advance of the new script setup sugar and full TypeScript support in Vue SFC templates. (This is totally optional but recommended as it results much better DX!)
            See
          ...
          <p>
            Vite exposes a special <code>meta.env</code> object for ENV variables (see <a href="https://vitejs.dev/guide/env-and-mode.html">official docs</a>). This template extends that object and
            adds custom typed variables which you can easily use and modify to your needs.
          </p>

          <p> See <code>vite.config.js</code> and <code>src/env.d.ts</code> for the configuration and <code>src/pages/BaseTemplate.vue</code> for usage example. </p>
          ...
          <p>
            ...
            <a href="https://caniuse.com/css-backdrop-filter"><em>backdrop-filter</em> CSS property</a>. The included <code>tailwind.config.js</code> has a custom plugin that allows you to use a
            <strong>'firefox' variant</strong> in your Tailwind markup to target FireFox. The setup uses Tailwind JIT mode and PurgeCSS so this doesn't add any weight to your CSS if you don't use it.
...
&lt;/div&gt;</pre
          ...
          <p> TL;DR: due to Vite and esbuild design decisions and ongoing work in this area, code coverage is not yet added to this template. </p>

          <p>
            I have a fully configured Vue 3 + TypeScript + Cypress + Coverage reporting + GitLab and GitHub CI `vue-cli` template project available at
            <a href="https://gitlab.com/uninen/vue-cli-template">https://gitlab.com/uninen/vue-cli-template</a>. That's a real world example of proper coverage configuration that works automatically
            in CI. But. This setup doesn't work with Vite.
          </p>

          <p>
            Typical Vite pipelines don't use babel at all which is needed above for automatically instrument the transpilated code. Vite is powered by eslint which has decided
            <a href="https://github.com/evanw/esbuild/issues/184"> code coverage being out of scope </a>. Vitest can be configured to produce code coverage as well but that only counts unit tests.
          </p>
...
<script setup lang="ts">
  ...
  const testValue = ['test', 'test2', 'test3', 'test4'];
